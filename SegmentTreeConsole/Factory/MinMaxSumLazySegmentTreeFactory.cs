...
{
    public class MinMaxSumLazySegmentTreeFactory : ISegmentTreeFactory<int[], int, int[]>
    {
        public GenericSegmentTree<int[], int, int[]> CreateSegmentTree(int[][] data)
        {
            // Made a lazy segment tree, with range query min, max, sum, and support range updates
            static ISegmentTreeNode<int[], int[]> lazyAttributesUpdater(ISegmentTreeNode<int[], int[]> curNode, int[] lazyUpdates)
            {
                var addLazyIndex = MinMaxSumLazySegmentTreeNode<int, int>.GetLazyAttributeIndex(AddUpdateLazyAttributeType.Add);

                // Special treatment for leaf node
                if (curNode.IsLeaf())
                {
                    var leafAttributesRef = curNode.GetAttributesRef();
                    var rawIndex = MinMaxSumLazySegmentTreeLeaf<int, int>.GetAttributeIndex(MinMaxSumLazySegmentTreeLeafAttributeType.Raw);
                    leafAttributesRef[rawIndex] += lazyUpdates[addLazyIndex];
                    return curNode;
                }
            ...
            static ISegmentTreeNode<int[], int[]> attributesUpdater(ISegmentTreeNode<int[], int[]> curNode, int update)
            {
                var attributesRef = curNode.GetAttributesRef();
            ...
            static ISegmentTreeNode<int[], int[]> childrenNodesCombinator(ISegmentTreeNode<int[], int[]> parent, ISegmentTreeNode<int[], int[]> leftChild, ISegmentTreeNode<int[], int[]> rightChild)
            {
            ...
            static int[] queryAttributesCombinator(int[]? left, int[]? right)
            {
                if (left == null)
                {
                    return right!;
            ...
            static ISegmentTreeNode<int[], int[]> treeNodeCreator(int[] value, bool isLeaf, int leftRange, int rightRange)
            {
                if (!isLeaf)
                {
                ...
                return new MinMaxSumLazySegmentTreeLeaf<int, int>([value[0]], [0], leftRange, rightRange);
            }

            return new GenericSegmentTree<int[], int, int[]>(
                    data,
